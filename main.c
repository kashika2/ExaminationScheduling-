...
#include <unistd.h>  //library for null pointer definition
#include <string.h>  //library for string functions
#include <stdbool.h>   //library for using boolean data type
#include<math.h>

#define MAX_COURSE_COUNT 32   //Max courses are defined as 32 using MACROS
#define MAX_COURSE_NAME_LENGTH 32  //Max name of the course is 32 using MACROS
...
typedef struct Graph_Vertex{             //typedef is used here to make  Graph_Vertex = Vertex;
    ...
    bool visited;
}Vertex;                        //A graph_vertex structure is created here



typedef struct Students             //Students structure is created here
{
    char course[32];
    int no;
}students;
...
char s[5];                 //char array "s"
bool isCourseExist(char *);          //Declaration of function
int **createEdges(FILE *);
int **createEdgeBetween(int **, char coursesOneStudentTakes[MAX_COURSE_COUNT][MAX_COURSE_NAME_LENGTH], int );     //Declaration of function
void readFromFile(FILE *);     //Declaration of function
void ReadFromFile2(FILE *,students **);      //Declaration of function
void RoomCalculator(int,students **);                              //Declaration of function
int getAdjUnvisitedVertex(int , int **, Vertex **);      //Declaration of function
void cleanUpVertex(Vertex *list[]);           //Declaration of function
void cleanUpEdge(int **);        //Declaration of function
void push(int);       //Declaration of function
int pop();          //Declaration of function
int peek();                         //Declaration of function
bool isStackEmpty();                             //Declaration of function
int determineColor(const int *);          //Declaration of function
Vertex *coloring(Vertex **, int **);                //Declaration of function
Vertex *DFS(Vertex **, int **, int);       //Declaration of function
Vertex *createVertexes(Vertex **);        //Declaration of function
Vertex *addVertex(char *);                                  //Declaration of function
Vertex *sortVertexes(Vertex **);      //Declaration of function

//Course initialization.
int courses[MAX_COURSE_COUNT][MAX_COURSE_NAME_LENGTH];       //A 2-D array of courses has been created here
int courseCount = 0;
...
char colors[10][10] = {{"Yellow"},{"Red"},{"Green"},{"Black"},{"Pink"},{"White"},{"Blue"},{"Brown"},{"Gray"},{"Orange"}};  //*******y 2d array *********//
...
int main() {
    ...
    int year;
    char str[32];
    FILE *fp1;   //file pointer for reading a file
    ...
    int **adjMatrix;    //double pointer is used here because adjMatrix will store 2d array
    
    //Create vertices with classes.
    *list = createVertexes(list); //After the execution of this line of code courseCount no. of nodes will be created. Each node will have name = coursename,color = Null, visited = false;
...
void readFromFile(FILE *fp)
{
    ...
    char input[256], word[256];     //input is the array for reading the string of each line from file,, word is the string
    char *token;
    ...
    while(!feof(fp))
    {
        ...
        {                                  //  printf("string is: %s\n", input);   //string is: Dhruv Gupta:MATHS,DAA,PDS , and more because eof is used in 155.input contain every string
            ...
            while(token != NULL)
            {
                ...
                //Token = DAA
                //  Token = PDS
                if(isCourseExist(token) == false)       //this function will check whether course already exists in the courses array
                {
...
Vertex *sortVertexes(Vertex **list){                //this function is used to sort the course names that are picked from the file
    
    char tempChar[32];            //a char array "tempChar" of size 32
